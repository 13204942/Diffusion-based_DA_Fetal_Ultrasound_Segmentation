...
def main():
    ...
    if args.model == "sam":
        with open(os.path.join(f'./logs/{SUB_LOGS}', f"{args.model}_AF_{args.samsize}_{current_time}_{args.eval_iter}.json"), 'w') as f:
            json.dump(logs, f)
    else:
        with open(os.path.join(f'./logs/{SUB_LOGS}', f"{args.model}_AF_{current_time}_{args.eval_iter}.json"), 'w') as f:
            json.dump(logs, f)
